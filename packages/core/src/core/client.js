    ...
    async tryCompressChat(force = false) {
        ...
        if (!force) {
            if (originalTokenCount === undefined) {
                ...
                console.warn(`Could not determine token count for model ${this.model}. Skipping compression check.`);
                return null;
            }
            ...
            if (!limit) {
                ...
                console.warn(`No token limit defined for model ${this.model}. Skipping compression check.`);
                return null;
            }
        ...
        const newTokenCount = (await this.getContentGenerator().countTokens({
            model: this.model,
            contents: newHistory,
        })).totalTokens;
